...
public class CustomDebug
{
    ...
    public static void DrawSphere(Vector4 pos, float radius, Color color, float lifetime)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            Debug.DrawLine(sX, eX, color, lifetime);
            Debug.DrawLine(sY, eY, color, lifetime);
            Debug.DrawLine(sZ, eZ, color, lifetime);
        }
    }
}
